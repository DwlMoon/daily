...
/**
 * String工具
 * 主要对 MoonStringUtils 的一些方法进行重写,达到更方便的使用
 * @author
 *
 */
@Slf4j
public class MoonStringUtils {
	...
	public static JSONObject isJSONObject(String args) {
		...
		try {
			return (JSONObject) JSONObject.parse(args.trim());
		} catch (Exception e) {
			return result ;
		}
	}
		...
		if(args instanceof  JSONArray){
			...
			for (Object json : arr) {
				if(json != null && json instanceof JSONObject){
					...
					result.add(JSONObject.parse((byte[]) json));
				}
			}
			return result;
		}else{
    ...
    public static String getBASE64(String str,boolean...bf) { 
       if (MoonStringUtils.isBlank(str)) return null;
	...
	public static List<String> array2Empty(String[] array){
		...
		for (String string : array) {
			if(MoonStringUtils.isNotBlank(string)){
				list.add(string);
			}
		}
		return list;
	...
	public static Integer stringToInteger( String intergerStr){
		Integer i =Integer.parseInt(intergerStr);
		return i;
	}
